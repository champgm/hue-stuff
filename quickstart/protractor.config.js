...
function Reporter(options) {
  const _defaultOutputFile = path.resolve(process.cwd(), './_test-output', 'protractor-results.txt');
  ...
  this.suiteDone = function (suite) {
    let statuses = _currentSuite.specs.map(spec => spec.status);
  ...
  this.jasmineDone = function () {
    ...
    const output = formatOutput(_root);
  ...
  function initOutputFile(outputFile) {
    ...
    const header = `Protractor results for: ${(new Date()).toLocaleString()}\n\n`;
    ...
    output.suites.forEach((suite) => {
      ...
      suite.specs.forEach((spec) => {
        ...
        if (spec.failedExpectations) {
          pad += indent;
          spec.failedExpectations.forEach((fe) => {
