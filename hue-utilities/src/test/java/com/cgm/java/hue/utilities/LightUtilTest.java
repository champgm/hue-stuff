...
public class LightUtilTest {
    ...
    public void testToggleLight() {
        ...
        Mockito.doReturn(lightOn).doReturn(lightOff).when(mockedHueBridgeGetter).getLight(hueConfiguration.getIP(), hueConfiguration.getToken(), lightId);
        ...
        final Light newLight = lightUtil.toggleLight(hueConfiguration.getIP(), hueConfiguration.getToken(), lightId);
        Assert.assertEquals(lightOff, newLight);
    }
}
