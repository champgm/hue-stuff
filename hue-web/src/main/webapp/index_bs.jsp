			...
			<div id="scenes" class="tab-pane fade">
				...
				<div class="row">
					<c:forEach items="${scenelist}" var="scene" varStatus="status">
						<div class="col-sm-6 col-md-4">
							<button type="button" id="${scene.getId()}"
								class="btn btn-default btn-lg">
								<c:out value="${scene.getName()}" />
							</button>
						</div>
					</c:forEach>
				</div>
			...
			<div id="groups" class="tab-pane fade">
				...
				<div class="row">
					<c:forEach items="${grouplist}" var="group" varStatus="status">
						<div class="col-sm-6 col-md-4">
							<c:choose>
								<c:when test="${group.getAction().getOn()}">
									<input type="checkbox" checked class="group"
										id="${group.getId()}" data-toggle="toggle"
										data-on="<br/>${group.getName()}"
										data-off="<br/>${group.getName()}" data-width="180"
										data-height="75" data-size="normal" data-style="ios"
										data-onstyle="success" data-offstyle="default">
								</c:when>
								<c:when test="${!group.getAction().getOn()}">
									<input type="checkbox" class="group" id="${group.getId()}"
										data-toggle="toggle" data-on="<br/>${group.getName()}"
										data-off="<br/>${group.getName()}" data-width="180"
										data-height="75" data-size="normal" data-style="ios"
										data-onstyle="success" data-offstyle="default">
								</c:when>
