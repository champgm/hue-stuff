						...
						<c:choose>
							<c:when test="${!light.getState().getOn()}">
								<button class="light-toggler" id="${light.getId()}">
									<img src="images/gray_light_icon.png" alt="" width=90px
										height=90px />
									<c:out value="${light.getName()}" />
								</button>
							</c:when>
							<c:when test="${light.getState().getOn()}">
			...
			<section id="tab2">
				...
				<p>
					<c:forEach items="${scenelist}" var="scene" varStatus="status">
						...
						<button class="scene" id="${scene.getId()}">
